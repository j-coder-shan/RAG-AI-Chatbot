...
- [ ] Start the Ollama service: `ollama serve` (leave running in its own
...
- [ ] Confirm both models are present: `ollama list`
- [ ] Confirm git is installed and configured: `git --version`
...
- [ ] Create `.gitignore`:
      ...
      *.egg-info/
      ...
      ```
- [ ] Create `.env.example` (see file provided alongside this plan —
...
- [ ] Create virtual environment and install:
      ...
      ```
- [ ] Copy `CLAUDE.md` (provided alongside this plan) into the repo root.
- [ ] Create `README.md` with at minimum: project description, setup
...
- [ ] `backend/config.py`: load all settings from environment variables
...
- [ ] `backend/loaders.py`: implement `load_pdf(file_path: str) -> str`
...
- [ ] `tests/test_loaders.py`: test that `load_pdf` raises `ValueError` on
      ...
      `tests/fixtures/`.
- [ ] `tests/test_chunking.py`: test that chunk count is sensible for a
      known input length, and that consecutive chunks share the configured
      overlap.
- [ ] Run `pytest tests/test_loaders.py tests/test_chunking.py -v` — all
      green.
- [ ] Commit: `git commit -am "feat: PDF loading and text chunking"`
...
- [ ] `backend/embeddings.py`: wrap `OllamaEmbeddings` from
...
- [ ] `backend/vectorstore.py`: implement using `langchain_chroma.Chroma`
      ...
      collection.
- [ ] **Critical correctness check:** confirm the same `EMBEDDING_MODEL`
...
- [ ] `tests/test_vectorstore.py`: with Ollama running, test that
...
- [ ] Run `pytest tests/test_vectorstore.py -v` — green (with Ollama
      running).
- [ ] Commit: `git commit -am "feat: Ollama embeddings + Chroma vector store"`
...
- [ ] `backend/rag_chain.py`: implement
...
- [ ] Handle the "no good match" case: if the top retrieved chunk's
...
- [ ] Write a small integration test in `tests/test_api.py` (or a
...
- [ ] Commit: `git commit -am "feat: RAG retrieval + generation chain"`
...
- [ ] `backend/routers/upload.py` — `POST /upload`:
...
- [ ] `backend/main.py`: instantiate `FastAPI()`, register both routers,
...
- [ ] `tests/test_api.py`: using `httpx.Client` against the FastAPI app
...
- [ ] Run `pytest -v` (full suite) — all green.
- [ ] Manually start the server and hit it with `curl` or the FastAPI
      `/docs` Swagger UI to confirm `/upload` then `/chat` works
      end-to-end with a real PDF.
- [ ] Commit: `git commit -am "feat: FastAPI upload and chat endpoints"`
...
- [ ] Manually test the full user flow: upload a real PDF, ask 2-3
      questions (including one with no answer in the doc), confirm
      sources display correctly.
- [ ] Commit: `git commit -am "feat: Streamlit chat UI"`
...
- [ ] Containerize the backend with Docker, but document clearly in
...
- [ ] Provide a `docker-compose.yml` that runs Ollama, the FastAPI
...
- [ ] Record a screen capture (Phase 8) showing the live local demo,
...
- [ ] Provision a small VM with enough RAM for the chosen model (e.g. an
...
- [ ] Install Docker + Docker Compose on the VM, clone the repo, run
      `docker compose up -d`.
- [ ] Open the necessary ports (8501 for Streamlit, 8000 for FastAPI) in
      the VM's firewall/security group.
- [ ] Put a reverse proxy (Caddy or Nginx) in front for HTTPS with a free
      Let's Encrypt cert if a clean URL matters for the portfolio.
- [ ] Set `BACKEND_URL` in the Streamlit service's environment to the
      VM's public backend address.

**Option C — Swap to a hosted free-tier LLM API for the public demo only**
- [ ] If neither A nor B is feasible, keep Ollama as the default/local
...
- [ ] Whichever option is used, update `README.md` with the exact
      run/deploy commands for that path.
- [ ] Commit: `git commit -am "docs: deployment instructions"`
...
- [ ] Add a short demo GIF or screen recording link showing: upload PDF →
      ask a question → see answer with sources → ask an unrelated
      question → see "not found" response.
- [ ] Final full run: `pytest -v` green, manual upload+chat smoke test
      passes, `docker compose up` (if Option A/B used) works from a clean
      clone in a scratch directory.
- [ ] Final commit and tag: `git commit -am "docs: finalize README and demo"`
      then `git tag v1.0`.
